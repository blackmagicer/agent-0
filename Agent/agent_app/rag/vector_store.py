...
from agent_app.model.factory import embed_model
from langchain_text_splitters import RecursiveCharacterTextSplitter
from agent_app.utils.path_tool import get_abs_path
from agent_app.utils.file_handler import pdf_loader, txt_loader, listdir_with_allowed_type, get_file_md5_hex
...
class VectorStoreService:
    ...
    def get_allowed_types(self) -> tuple[str, ...]:
        return tuple(chroma_conf["allow_knowledge_file_type"])
    ...
    def reset_store(self):
        persist_dir = self.get_persist_dir()
        if os.path.exists(persist_dir):
            shutil.rmtree(persist_dir)
        md5_store = self.get_md5_store_path()
    ...
    def load_document(self):
        ...
        for file_id, path in enumerate(allowed_files_path, start=1):
            # 获取文件的MD5
