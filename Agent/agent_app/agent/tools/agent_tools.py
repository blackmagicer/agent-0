...
import random
from agent_app.utils.config_handler import agent_conf
...
def _gaode_get(path: str, params: dict) -> dict:
    gaode_key = (os.getenv("GAODE_KEY") or "").strip()
    if not gaode_key:
...
@tool(description="根据问题检索知识库中的候选参考资料，以纯字符串形式返回")
def query_knowledge_base(query: str) -> str:
    return rag.query_knowledge_base(query)


@tool(description="获取指定文件ID的元信息，以纯字符串形式返回")
def get_files_meta(fileIds: list[int]) -> str:
    return rag.get_files_meta(fileIds)


@tool(description="读取指定文件chunk内容，以纯字符串形式返回")
def read_file_chunks(chunks: list[dict[str, int]]) -> str:
    return rag.read_file_chunks(chunks)


@tool(description="列出知识库中的文件列表，以纯字符串形式返回")
def list_files(page: int = 0, pageSize: int = 10) -> str:
    return rag.list_files(page, pageSize)


@tool(description="使用 Agentic RAG 强制经过四工具链路检索并总结")
def rag_summarize(query: str) -> str:
    return rag.agentic_rag(query)
